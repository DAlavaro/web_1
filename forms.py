...
class UserForm(forms.Form):
    ...
    # datetime = forms.DateTimeField(label="Введите дату и время")
    # time_delta = forms.DurationField(label="Введите промежуток времени")

    # num = forms.DecimalField(label="Введите целое число", decimal_places=3)

    # sities = ((1, "Москва"), (2, "Воронеж"), (3, "Курск"))
    # city = forms.TypedChoiceField(label="Выберите город", empty_value=None, choices=sities)
    ...
    countries = (
        ...
        (3, "Spanish"),
        (4, "Russia"))
    country = forms.MultipleChoiceField(label="выберите Страну", choices=countries)
